    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Select
          ...
          options={INCOME_SOURCES} placeholder="Select source"
        ...
        <div className="sm:col-span-2">
          <Input label="Reference / Booking ID" placeholder="e.g. BK-ABC123" {...register('reference')} />
        </div>
      </div>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Select
          ...
          options={EXPENSE_CATEGORIES} placeholder="Select category"
...
function IncomeTab() {
  ...
  const filtered = filterBySearch(data, search, ['source', 'description', 'reference', 'paymentMethod']);
    ...
    <>
      ...
      {loading ? <LoadingSpinner /> : filtered.length === 0 ? (
        <EmptyState
          ...
          description={search ? 'Try a different search' : 'Record your first income entry'}
          action={!search && <Button onClick={openAdd} size="sm"><Plus size={14} /> Add Income</Button>}
        />
      ) : (
...
function ExpenseTab() {
  ...
  const filtered = filterBySearch(data, search, ['category', 'description', 'paidTo', 'paymentMethod']);
    ...
    <>
      ...
      {loading ? <LoadingSpinner /> : filtered.length === 0 ? (
        <EmptyState
          ...
          description={search ? 'Try a different search' : 'Record your first expense'}
          action={!search && <Button variant="danger" onClick={openAdd} size="sm"><Plus size={14} /> Add Expense</Button>}
        />
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                {['Date', 'Category', 'Description', 'Paid To', 'Payment', 'Amount', 'Actions'].map((h) => (
                  <th key={h} className={`px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide whitespace-nowrap ${h === 'Actions' ? 'text-right' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {filtered.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-5 py-3 text-gray-600 whitespace-nowrap">{formatDate(r.date)}</td>
                  <td className="px-5 py-3 font-medium text-gray-800 whitespace-nowrap">{r.category}</td>
                  <td className="px-5 py-3 text-gray-500 max-w-xs truncate">{r.description || '—'}</td>
