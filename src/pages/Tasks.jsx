...
export default function Tasks() {
  ...
  const visibleTasks = useMemo(() => {
    if (isEmployee && profile?.name) {
      return tasks.filter((t) => t.assignedTo === profile.name);
    }
    return tasks;
  ...
  const stats = useMemo(() => ({
    ...
    todo: visibleTasks.filter((t) => (t.status || 'To Do') === 'To Do').length,
      ...
      <div className="bg-white rounded-xl border border-gray-100">
        ...
        {loading ? <LoadingSpinner /> : filtered.length === 0 ? (
          <EmptyState
            ...
            description={isEmployee ? 'No tasks assigned to you yet' : 'Create your first task'}
